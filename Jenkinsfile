            ...
            steps {
                ...
                withKubeConfig([credentialsId: 'kubernetes-service-account']) {
                    script {
                        ...
                        sh './kubectl get Secret cts-inventory-${BRANCH_NAME}-database -n cts-inventory || sed "s/REPLACEME_NAME/cts-inventory-${BRANCH_NAME}-database/g; s/REPLACEME_PASSWORD/$(echo -n $(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 32) | base64)/g" ./jenkins/secret.yaml | ./kubectl apply -f -'
