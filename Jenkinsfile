            ...
            steps {
                ...
                withKubeConfig([credentialsId: 'kubernetes-service-account']) {
                    script {
                        ...
                        sh './kubectl get Secret cts-inventory-${BRANCH_NAME}-database -n cts-inventory || sed "s/REPLACEME_NAME/cts-inventory-${BRANCH_NAME}-database/g; s/REPLACEME_PASSWORD/$(echo $(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 32 ; echo "") | base64)/g" ./jenkins/secret.yaml | ./kubectl apply -f -'
                        ...
                        sh './kubectl get ConfigMap cts-inventory-${BRANCH_NAME}-server -n cts-inventory || sed "s/REPLACEME_NAME/cts-inventory-${BRANCH_NAME}-server/g; s/REPLACEME_ENVIRONMENT/${BRANCH_NAME}/g; s/REPLACEME_PASSWORD/$(./kubectl get secret cts-inventory-${BRANCH_NAME}-database -n cts-inventory --template={{.data.password}} | base64 -d)/g; s/REPLACEME_SESSION_PASSWORD/$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 32 ; echo "")/g; s/REPLACEME_HOST_DATABASE/cts-inventory-${BRANCH_NAME}-database/g; s/REPLACEME_HOST_REDIS/cts-inventory-${BRANCH_NAME}-redis/g" ./jenkins/configMap.yaml | ./kubectl apply -f -'
